    ...
    for (let row = 0; row < 4; row++) {
        ...
        for (let col = 0; col < 27; col++) {
            ...
            if (index < foodItems.length) {
                ...
                foodImg.src = foodItems[index].img;
                foodImg.alt = foodItems[index].name;
    
                const foodText = document.createElement("div");
                foodText.textContent = foodItems[index].name;
                ...
                const imageUrl = foodItems[index].img;
    
                // If image was already loaded before, skip shimmer
                if (sessionStorage.getItem(imageUrl)) {
                    foodDiv.appendChild(foodImg);
                    foodDiv.appendChild(foodText);
                } else {
                    foodDiv.classList.add("skeleton"); // add shimmer effect
                    foodDiv.appendChild(foodImg);
                    foodDiv.appendChild(foodText);
                    ...
                    imgLoader.onload = () => {
                        sessionStorage.setItem(imageUrl, "true");
                        foodDiv.classList.remove("skeleton");
